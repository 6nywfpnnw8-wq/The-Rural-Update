(function () {
  const signals = [
    {
      id:'regionalization', title:'Rural regionalization', direction:'Increasing', confidence:'High', horizon:'1–5 years', executiveRisk:'Opportunity',
    ...
    {
      id:'medicaid-financing', title:'Medicaid financing pressure', direction:'Increasing', confidence:'High', horizon:'12–48 months', executiveRisk:'High',
      ...
      why:'Provider taxes, state-directed payments, DSH, base Medicaid reimbursement and uncompensated care should be evaluated together because changes in one financing mechanism can alter the economics of the others.',
      refs:[
    ...
    {
      id:'rhc-sustainability', title:'RHC sustainability opportunities', direction:'Improving', confidence:'Medium', horizon:'6–18 months', executiveRisk:'Opportunity',
      ...
      why:'Transformation projects are more durable when grant-funded program development can transition into ordinary reimbursement. This proposal is a concrete test of that bridge.',
      refs:[
    ...
    {
      id:'payer-friction', title:'Payer administrative friction', direction:'Potentially improving', confidence:'Medium', horizon:'3–12 months', executiveRisk:'Moderate',
    ...
    {
      id:'workforce-localization', title:'Workforce localization', direction:'Increasing', confidence:'High', horizon:'1–5 years', executiveRisk:'Opportunity',
      ...
      why:'Programs that build local residency, rotation, preceptor and education infrastructure create a stronger pathway from temporary grant support to durable clinical capacity in rural communities.',
      refs:[
  ...
  function renderSignals(){
    const section=document.getElementById('page-signals'); if(!section)return;
    if(!document.getElementById('signal-library-styles')){
      const style=document.createElement('style'); style.id='signal-library-styles';
      style.textContent='.signal-index{display:grid;gap:8px}.signal-link{display:block;text-decoration:none;color:inherit;border:1px solid var(--border);border-radius:10px;padding:11px 12px;background:var(--card)}.signal-link:hover,.signal-link:focus{border-color:var(--gold)}.signal-link strong{display:block;color:var(--navy);font-size:13px}.signal-link span{display:block;color:var(--gray);font-size:11.5px;margin-top:2px}.signal-link em{float:right;color:var(--gold);font-style:normal;font-weight:800}.signal-meta{display:flex;flex-wrap:wrap;gap:5px;margin-top:7px}.signal-chip{display:inline-block;font-size:9px;border-radius:999px;background:var(--soft);color:var(--navy);padding:3px 7px;font-weight:800}.evidence-card{scroll-margin-top:70px}.evidence-card h3{font-size:15px;color:var(--navy);margin:0 0 4px}.evidence-card p{font-size:12.5px;margin:0 0 9px}.evidence-card h4{font-size:10px;text-transform:uppercase;letter-spacing:.06em;color:var(--gray);margin:13px 0 6px}.reading-link{display:block;padding:8px 9px;border-top:1px solid var(--border);color:var(--navy2);font-size:11.5px;text-decoration:none}.reading-link:first-of-type{border-top:0}.reading-link:after{content:" ↗"}.back-signals{display:inline-block;margin-top:10px;font-size:10.5px;color:var(--navy2);text-decoration:none;font-weight:700}';
      document.head.appendChild(style);
    }
    let html='<div class="card"><h2>Signals &amp; Trends</h2><p class="meta" style="margin:0 0 10px;">Directional assessments are paired with confidence, time horizon, executive risk and supporting evidence. These are public-source indicators, not forecasts.</p><div class="signal-index">';
    signals.forEach(function(s){html+='<a class="signal-link" href="'+signalHref(s.id)+'"><em>›</em><strong>'+esc(s.title)+': '+esc(s.direction)+'</strong><span>'+esc(s.summary)+'</span><div class="signal-meta"><span class="signal-chip">Confidence: '+esc(s.confidence)+'</span><span class="signal-chip">Horizon: '+esc(s.horizon)+'</span><span class="signal-chip">Executive risk: '+esc(s.executiveRisk)+'</span></div></a>';});
    html+='</div></div><div class="card"><h2>Executive Interpretation</h2><p>The strategic pattern remains consistent: rural sustainability is moving toward regional capability, differentiated local services, stronger payer intelligence and explicit post-grant operating models. Confidence and time horizon help distinguish established structural movement from emerging possibilities.</p></div><div id="signal-evidence"><div class="card"><h2>Signal Evidence &amp; Reading Library</h2><p class="meta">Use these references to validate the signal, understand the underlying policy or market change, and read beyond the daily brief.</p></div>';
    signals.forEach(function(s){
      html+='<div class="card evidence-card" id="signal-'+esc(s.id)+'"><h3>'+esc(s.title)+'</h3><div class="signal-meta"><span class="signal-chip">Direction: '+esc(s.direction)+'</span><span class="signal-chip">Confidence: '+esc(s.confidence)+'</span><span class="signal-chip">Horizon: '+esc(s.horizon)+'</span><span class="signal-chip">Executive risk: '+esc(s.executiveRisk)+'</span></div><p><strong>What the signal indicates:</strong> '+esc(s.summary)+'</p><p><strong>Why it matters:</strong> '+esc(s.why)+'</p><h4>Evidence &amp; Further Reading</h4>';
      s.refs.forEach(function(r){html+='<a class="reading-link" href="'+esc(r[1])+'" target="_blank" rel="noopener">'+esc(r[0])+'</a>';});
    ...
    homepageSignals.forEach(function(node){
      const text=node.textContent.toLowerCase(); let id=null;
      if(text.includes('medicaid'))id='medicaid-financing'; else if(text.includes('regional'))id='regionalization'; else if(text.includes('payer'))id='payer-friction'; else if(text.includes('workforce'))id='workforce-localization';
      if(!id)return; node.style.cursor='pointer'; node.setAttribute('role','link'); node.setAttribute('tabindex','0');
      const go=function(){const btn=document.querySelector('.tab-btn[onclick*="signals"]'); if(btn&&typeof window.showTab==='function')window.showTab('signals',btn); setTimeout(function(){const target=document.getElementById('signal-'+id); if(target)target.scrollIntoView({behavior:'smooth',block:'start'});},50);};
      node.addEventListener('click',go); node.addEventListener('keydown',function(e){if(e.key==='Enter'||e.key===' '){e.preventDefault();go();}});
    });
  }

  window.RuralUpdateSignals=signals;
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',renderSignals); else renderSignals();
})();
