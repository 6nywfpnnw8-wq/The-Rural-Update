...
def add_text(parent, tag, text, ns=None):
    element = ET.SubElement(parent, q(ns, tag) if ns else tag)
    element.text = text
    return element
...
def main():
    ...
    for audio_path in sorted(audio_dir.glob("*.mp3"), reverse=True):
        ...
        transcript_path = transcript_dir / f"{date}.html"

        if not script_path.exists():
            continue

        meta, _body, intro = parse_script(script_path)
        ...
        if transcript_path.exists():
            episode["transcript_url"] = f"{BASE_URL}/transcript/{date}.html"
    ...
    add_text(
        channel,
        "description",
        "A concise daily executive briefing on rural healthcare policy, operations, finance, Oklahoma developments, federal policy, and emerging technology.",
    )
    ...
    add_text(
        channel,
        "summary",
        "Daily rural healthcare executive intelligence in under 15 minutes.",
        ITUNES,
    )
    ...
    for episode in episodes:
        ...
        add_text(item, "encoded", episode["description"], CONTENT)
        add_text(item, "pubDate", format_datetime(episode["pub_dt"]))

        guid = add_text(item, "guid", f"the-rural-update-{episode['date']}")
        ...
        add_text(item, "duration", episode["duration"], ITUNES)
        add_text(item, "episodeType", "full", ITUNES)
        add_text(item, "explicit", "false", ITUNES)
        add_text(item, "summary", episode["description"], ITUNES)

        if "transcript_url" in episode:
            ET.SubElement(
                item,
                q(PODCAST, "transcript"),
                {
